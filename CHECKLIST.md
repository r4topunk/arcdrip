...
| 22 | 10 | Leave pool 1 streaming **≥ 3 days**, then reconcile with the SDK | `pnpm --filter '@sharedarc/sdk...' build`; then a node one-liner over `getPool` / `getMember` / `claimable` (SDK `math.ts`) | `Σ withdrawn + Σ claimable + dust == streamed`, dust < 1e-6 USDC (PRD §13) | [ ] |
